				 ...
				 unsigned long npages)
{
	...
	slots = kvzalloc(sizeof(*slots), GFP_KERNEL);
	if (!slots)
		return -ENOMEM;
	...
	for (i = 0; i < old_slots->used_slots; i++) {
		...
		if (i == 0) {
			start = base_vfn;
			end = min(base_vfn + npages,
				  old_slots->memslots[i].base_vfn);
		} else {
			old_slot = &old_slots->memslots[i - 1];
			if (base_vfn + npages <=
			    old_slot->base_vfn + old_slot->npages)
				continue;
			start = max(base_vfn,
				    old_slot->base_vfn + old_slot->npages);
			end = min(base_vfn + npages,
			...
			int as_id)
{
	...
	for (gfn = slot->base_gfn; gfn < gfn_end; gfn += npages) {
		...
		for (i = 0; i < slots->used_slots; i++) {
			...
			while (iter_idx >= 0) {
				...
				gfn_iter = __kvm_dsm_vfn_to_gfn(
					hvaslot, true, hvaslot->base_vfn,
					&is_smm, &iter_idx, NULL);
				if (!!is_smm == !!as_id && gfn_iter <= gfn &&
				    gfn_iter + hvaslot->npages > gfn) {
					flag = true;
					break;
				}
			}
		...
		if (new_hvaslot == hvaslot)
			continue;
		npages = min(gfn_end - gfn,
			     min(hvaslot->npages - (gfn - gfn_iter),
				 new_hvaslot->npages -
					 (vfn - new_hvaslot->base_vfn)));
		/* Retrieve dsm state from backup in case of gfn->vfn mapping changes. */
		for (i = 0; i < npages; i++) {
			new_hvaslot
				->vfn_dsm_state[i +
						(vfn - new_hvaslot->base_vfn)]
				.state =
				hvaslot->vfn_dsm_state[i + (gfn - gfn_iter)]
					.state;
			memcpy(new_hvaslot
				       ->vfn_dsm_state[i +
						       (vfn -
							new_hvaslot->base_vfn)]
				       .copyset,
			       hvaslot->vfn_dsm_state[i + (gfn - gfn_iter)]
				       .copyset,
			       sizeof(copyset_t));
		}
		kvm_dsm_rmap_remove(kvm, hvaslot, true,
				    (gfn << 1) | GFN_PRESENT_MASK |
					    (as_id ? GFN_SMM_MASK : 0),
				    hvaslot->base_vfn + (gfn - gfn_iter),
				    npages);
	}
	...
	for (gfn = slot->base_gfn; gfn < gfn_end; gfn += npages) {
		...
		if (!hvaslot)
			return 0;
		npages = min(gfn_end - gfn,
			     hvaslot->base_vfn + hvaslot->npages - vfn);
...
out:
	if (i == kvm_arch_nr_memslot_as_ids(kvm))
		return;
 ...
 */
static int __kvm_dsm_acquire_page(struct kvm *kvm, struct kvm_memory_slot *slot,
				  gfn_t gfn, bool is_smm, bool write)
{
	struct kvm_dsm_memory_slot *hvaslot;
	hfn_t vfn;
	int dsm_access;
			      ...
			      bool write)
{
	...
	if (slot)
		*slot = memslot;
	return __kvm_dsm_acquire_page(vcpu->kvm, memslot, gfn, is_smm(vcpu),
				      write);
}
			 ...
			 gpa_t gpa, unsigned long len, bool write)
{
	...
	for (gfn = gpa >> PAGE_SHIFT; gfn <= gfn_end; gfn++) {
		slot = __gfn_to_memslot(memslots, gfn);
		ret = __kvm_dsm_acquire_page(vcpu->kvm, slot, gfn, is_smm(vcpu),
					     write);
		if (ret < 0)
			goto out_release;
	}
...
static int kvm_dsm_threadfn(void *data)
{
	...
	pr_devel("dsm: kvm[%d] started dsm server on %s:%u\n", kvm->arch.dsm_id,
		 addr.host, addr.port);
	count = 0;
	while (1) {
		...
		pr_devel("kvm-dsm: node-%d accepted connection\n",
			 kvm->arch.dsm_id);
		conn->kvm = kvm;
		...
		for (i = 0; i < NDSM_CONN_THREADS; i++) {
			 ...
			 */
			thread = kthread_run(kvm_dsm_handle_req, (void *)conn,
					     "dsm-conn/%d:%d", kvm->arch.dsm_id,
					     count++);
			if (IS_ERR(thread)) {
				pr_err("kvm-dsm: failed to start kernel thread for dsm connection\n");
				ret = PTR_ERR(thread);
				goto out_accept_sock;
			}
...
out_listen_sock:
	for (i = 0; i < DSM_MAX_INSTANCES; i++) {
		...
		if (kvm->arch.dsm_conn_socks[DSM_MAX_INSTANCES + i])
			network_ops.release(
				kvm->arch.dsm_conn_socks[DSM_MAX_INSTANCES + i]);
	}
	...
	while (!list_empty(&conn_list)) {
		...
		for (i = 0; i < NDSM_CONN_THREADS; i++) {
			...
			dsm_debug(
				"kvm[%d] dsm connection thread %s exited with %d",
...
static int kvm_dsm_init(struct kvm *kvm, struct kvm_dsm_params *params)
{
	...
	kvm->arch.cluster_iplist =
		kcalloc(params->cluster_iplist_len, sizeof(void *), GFP_KERNEL);
	...
	WARN_ON(copy_from_user(user_cluster_iplist, params->cluster_iplist,
			       sizeof(void *) * params->cluster_iplist_len) !=
		0);
	for (i = 0; i < params->cluster_iplist_len; i++) {
		kvm->arch.cluster_iplist[i] = kmalloc(20, GFP_KERNEL);
		if (!kvm->arch.cluster_iplist[i]) {
			ret = -ENOMEM;
			goto out_free_cluster_iplist;
		}
		if (strncpy_from_user(kvm->arch.cluster_iplist[i],
				      user_cluster_iplist[i], 20) == 20) {
			ret = -EINVAL;
			goto out_free_cluster_iplist;
		}
	}
	...
	kvm->arch.dsm_conn_socks = kcalloc(DSM_MAX_INSTANCES * 2,
	...
	thread = kthread_run(kvm_dsm_threadfn, (void *)kvm, "kvm-dsm/%d",
			     kvm->arch.dsm_id);
	if (IS_ERR(thread)) {
		pr_err("kvm-dsm: failed to start kernel thread for dsm server\n");
		ret = PTR_ERR(thread);
		goto out;
	}
...
int kvm_dsm_alloc(struct kvm *kvm)
{
	...
	kvm->arch.dsm_hvaslots = kvm->arch.dsm_hvaslots =
		kvzalloc(sizeof(*kvm->arch.dsm_hvaslots), GFP_KERNEL);
...
void kvm_dsm_free(struct kvm *kvm)
{
	...
	if (kvm->arch.dsm_enabled && kvm->arch.dsm_thread) {
		pr_devel("kvm-dsm: node-%d stopping dsm server\n",
			 kvm->arch.dsm_id);
		kvm->arch.dsm_stopped = true;
		/* Ensure dsm_enabled is visible before checking dsm_thread */
		smp_mb();
		send_sig(SIGKILL, kvm->arch.dsm_thread, 1);
		ret = kthread_stop(kvm->arch.dsm_thread);
		if (ret < 0) {
			pr_err("%s: node-%d dsm root server exited with %d\n",
...
static int kvm_dsm_page_fault(struct kvm *kvm, struct kvm_memory_slot *memslot,
			      gfn_t gfn, bool is_smm, int write)
{
	return ivy_kvm_dsm_page_fault(kvm, memslot, gfn, is_smm, write);
}

int kvm_dsm_memcpy(struct kvm *kvm, unsigned long host_virt_addr,
		   unsigned long userspace_addr, unsigned long length,
		   bool write)
{
	...
	pr_devel("dsm: hva %lx, length %lu, write %d\n", host_virt_addr, length,
		 write);
	...
	for (vfn = host_virt_addr >> PAGE_SHIFT; vfn < vfn_end; vfn += npages) {
		...
		npages =
			min(vfn_end - vfn, slot->base_vfn + slot->npages - vfn);
		...
		for (i = 0; i < npages; i += gfn_npages) {
			gfn = __kvm_dsm_vfn_to_gfn(slot, false, vfn + i,
						   &is_smm, NULL, NULL);
			memslot = __gfn_to_memslot(__kvm_memslots(kvm, is_smm),
						   gfn);
			gfn_npages =
				min(npages - i,
				    (unsigned long)(memslot->base_gfn +
						    memslot->npages - gfn));
			for (j = 0; j < gfn_npages; j++) {
				ret = kvm_dsm_page_fault(kvm, memslot, gfn + j,
							 is_smm, write);
				if (ret < 0)
					goto out_unlock;
			}
...
do_memcpy:
		hva_start =
			max(host_virt_addr, (unsigned long)(vfn << PAGE_SHIFT));
		user_buf =
			(void *)(userspace_addr + hva_start - host_virt_addr);
		if (slot) {
			hva_end = min(
				host_virt_addr + length,
				(unsigned long)(slot->base_vfn + slot->npages)
					<< PAGE_SHIFT);
		} else {
			hva_end = min(host_virt_addr + length,
				      (unsigned long)(vfn + 1) << PAGE_SHIFT);
		}
		/*
		   ...
		   unsigned long length, bool write, bool unpin)
{
	...
	pr_devel("dsm: hva %lx, length %lu, write %d unpin %d\n",
...
long kvm_vm_ioctl_dsm(struct kvm *kvm, unsigned int ioctl, unsigned long arg)
{
	...
	case KVM_DSM_IPI: {
		...
		kvm_lapic_reg_write_remote(vcpu->arch.apic, params.val,
					   params.val2, params.dest_id);
		break;
	}
	case KVM_DSM_X2APIC: {
	...
	case KVM_DSM_APIC_BASE: {
		...
		struct msr_data msr_info = {
			.host_initiated = params.host,
			.index = params.index,
			.data = params.data,
		};

		kvm_apic_set_base(vcpu, &msr_info.data,
	...
	case KVM_DSM_MEMPIN: {
		...
		if (copy_from_user(&pin, argp, sizeof(pin)))
			goto out;
		r = kvm_dsm_mempin(kvm, pin.host_virt_addr, pin.length,
				   pin.write, pin.unpin);
		if (r)
			goto out;
		break;
